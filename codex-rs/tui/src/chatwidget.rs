...
const AUTO_NEXT_STEPS_META_TEMPLATES: &[&str] = &[
    "Continue: You just finished the work above. Based specifically on what was accomplished, what is the single most impactful next step? Don't go generic -- look at the actual changes made and determine what naturally follows. Execute it.",
    "Continue: Review the work you just completed above. What did it leave unfinished, what edge cases remain, what would a thorough developer do next given exactly this context? Pick the highest-value action and do it.",
    "Continue: Look at your last output and the state of the project after your changes. What's the most important thing that needs to happen next? Be specific to what you actually did -- not generic advice. Act on it.",
    "Continue: Given what you just accomplished, assess: did it introduce anything that needs testing? Does it enable a follow-up improvement? Is there cleanup needed? Pick whichever is most valuable and execute.",
];

const AUTO_NEXT_IDEA_META_TEMPLATES: &[&str] = &[
    "Continue: Based on everything you've seen in this project so far, what's the most impactful improvement you could make right now? Don't repeat previous work. Pick something specific, explain why it matters in this context, and implement it.",
...
impl ChatWidget {
    ...
    fn spawn_auto_next_prompt_generation(
        ...
        let Some(thread_id) = self.thread_id else {
            self.queued_user_messages
                .push_back(UserMessage::from(fallback_prompt).into());
            return;
        };

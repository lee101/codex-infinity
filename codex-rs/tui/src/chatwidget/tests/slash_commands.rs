...
#[tokio::test]
async fn model_slash_command_is_available_while_task_is_running() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert!(
        ...
        "expected model picker, got: {rendered:?}"
    );
    assert!(
        cells.is_empty(),
        "expected no disabled-command history message, got {} cells",
        cells.len()
    );
